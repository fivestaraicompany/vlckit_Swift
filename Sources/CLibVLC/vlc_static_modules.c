/*
 * vlc_static_modules.c
 *
 * Defines the `vlc_static_modules` table and provides a `memrchr`
 * implementation that the prebuilt MobileVLCKit static library
 * references but does not bundle. Without this file, consumer apps fail
 * to link with errors like:
 *   Undefined symbol: _vlc_static_modules
 *   Undefined symbol: _memrchr
 *
 * Originally generated by buildMobileVLCKit.sh from
 * Resources/MobileVLCKit/vlc-plugins-iPhone.h. Inlined here so the SPM
 * package is self-contained.
 *
 * `memrchr` is a GNU extension that is referenced by libvlccore but
 * absent from Apple platforms; we re-export a minimal implementation.
 *
 * Compiled only on Apple/iOS-style targets where the prebuilt static
 * archive is consumed.
 */
...
/* memrchr — search for byte `c` in last `n` bytes of `s`, returning the
 * last match. */
...
/* --- Static module entry-point declarations ----------------------------- */
...
int vlc_entry__services_discovery_libupnp (int (*)(void *, void *, int, ...), void *);
...
int vlc_entry__audio_filter_libebur128 (int (*)(void *, void *, int, ...), void *);
int vlc_entry__codec_dvbsub (int (*)(void *, void *, int, ...), void *);
int vlc_entry__control_dummy (int (*)(void *, void *, int, ...), void *);
int vlc_entry__access_dsm_libdsm (int (*)(void *, void *, int, ...), void *);
...
const void *vlc_static_modules[] = {
    ...
    vlc_entry__stream_out_transcode_libstream_out_transcode,
    ...
    vlc_entry__video_filter_magnify,
    ...
    vlc_entry__demux_libdiracsys,
    vlc_entry__demux_demuxdump,
    ...
    vlc_entry__stream_filter_cache_block,
    ...
    vlc_entry__audio_output_audiounit_ios,
